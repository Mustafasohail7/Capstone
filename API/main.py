...
def main(file_path,start_date,end_date,download_all):
    ...
    for i in range(len(coordinates)):
        ...
        landsat = Landsat(baseUrl,datasetName,coordinates[i][0],coordinates[i][1],start_date,end_date,download_all)
        retries = 0
...
if __name__ == "__main__":
    ...
    start_date = sys.argv[2] if len(sys.argv) > 2 else datetime.date.today().strftime('%Y-%m-%d')
    end_date = sys.argv[3] if len(sys.argv) > 3 else ''
    download_all = sys.argv[4] if len(sys.argv) > 4 else False
    main(file_path,start_date,end_date,download_all)